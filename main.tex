...
\begin{itemize}
\item
Die Komponenten der Basalganglien aufzählen
\item
Go- und No Go-Wege der Basalganglien erklären
\item
Für Bewegungen releavante Areale des Motorkortex benennen und beschreiben
\item 
Die Rollen des motorishen Hirnstamms benennen
\item
Deszendierende und Aszendierende Bahnen des Motorsystems erläutern
\item
Transkortikale Reflexe erklären
\item
Anhand eines Beispiels erläutern, wie motorische Strukturen zusammenwirken
\end{itemize}
...
Morbus Parkinson auf der Ebene der Basalganglien erklären
\item
Morbus Huntington  auf der Ebene der Basalganglien erklären
\item
Symptome bei Schädigung deszendierender Bahnen benennen
\end{itemize}
...
\begin{center}
    \includegraphics[width=\textwidth]{jogger.jpg}
\end{center}
...
\begin{frame}{Willkürbewegungen sind gelernt}

  \begin{columns}[c]

\begin{column}{5cm}

Die zeitliche und örtliche Koordination von funktioneller Muskelgruppen muss erlernt werden. Dies passiert vor allem in der Kindheit, ist aber bis ins hohe Alter möglich. Rückkopplungsmechanismen durch afferentes Feedback erlauben Korrekturen des Beweungsablaufs
...
\begin{center}
    \includegraphics[width=\textwidth]{toddler.jpg}
\end{center}
...
\begin{column}{3cm}
        Direkter Weg: \\ Go  \\ (fördert Bewegung) \\[0.2 cm]
        Indirekter Weg: NoGo  \\ (hemmt Bewegung) \\

\end{column}

\begin{column}{8cm}

\includegraphics<1>[width=\textwidth]{Basalganglien_direkt.png}

\includegraphics<2>[width=\textwidth]{Basalganglien_direkt_indirekt.png}
...
Das Striatum erhält Eingänge aus den Assoziationarealen und sensorischen Arealen des Kortex
    \item 
    Zu \(90\,\%\) mittelgroße Projektionsneuronen (Medium Spiny Neurons, MSN)
    \item
    MSNs sind GABAerg
    \item
    ...
    \begin{itemize}
        \item 
        Substanz P (SP): Go-Weg
        \item
        Enkaphalin (Enk): NoGo-Weg
...
\begin{frame}{Regulierung des Striatums durch die SNc}
...
\begin{center}
\includegraphics[width=0.8\textwidth]{Basalganglien_all.png}
...
Kontralaterale Repräsentation (Ausnahmen: Mimische Muskulatur Stirnregion, Zungen-, Kau- und Schluckmuskulatur sind bilateral repräsentiert) \\
...
\begin{itemize}
    ...
    \item
    Antizipatorische Kraftkontrolle ("Wie schwer ist das Objekt, das ich heben möchte")
    \item
...
\begin{frame}{Motorischer Hirnstamm}
    ...
    \begin{column}{4cm}
\begin{center}
    \includegraphics[width=\textwidth]{brain_stem.png}
\end{center}    
    \end{column}
...
\begin{frame}{Pyramidenbahn (Kortikospinaler Trakt)}

Efferente Bahnen signalisieren Durchführung, Planung und Initiierung von Bewegungen \\
...
Axone von Pyramidenzellen (Schicht III und V) \\
    \(40\,\%\) primär-motorischer Kortex,   \(30\,\%\) prämotorischer Kortex, \(30\,\%\) v.a. S1 \\
    Verläuft in der Capusla Interna \\
    \(85\,\%\) kreuzen in Pyramide auf andere Seite über (lateraler Kortikospinaltrakt), der Rest bleibt ipsilateral (anteriorer Kortikospinaltrakt)\\
    Klein: nur ca. 1 Million Axone pro Seite \\
...
\begin{column}{8cm}
    ...
    Innerviert hauptsächlich* spinale Interneurone, die dann \(\alpha\)-Motoneuronen kontaktieren \\

Ausnahme: Betzsche Riesenzellen innervieren direkt \(\alpha\)-Motoneuronen der Finger und erlauben dadurch den präzisen Pinzettengriff \\


Kollateralle Fasern erlauben es, mehr als ein Motoneuron pro Efferenz zu stimulieren. Kollaterale signalisieren auch in den Hirnstamm zur unteren Olive 
...
\begin{frame}{Weitere deszendierende Bahnen}

\begin{block}{Kortiko-bulbäre (kortiko-nukleäre) Bahn}

\begin{itemize}
    \item 
    Verläuft in der Capsula interna
    \item
    Zieht zu sensorischen und motorischen Hirnnerven
    \begin{itemize}
        \item 
        direkte Innervierung von V, VII IX, und XII
        \item
        Mimik, Sprachmuskulatur, Nahrungsaufnahme
        \item
        Bilateral bis auf unteren Gesichtsanteil
    \end{itemize}
\end{itemize}
...
\begin{block}{Kortiko-thalamische Projektionen}

\begin{itemize}
    \item 
    Verarbeitung beweungsrelevanter afferenter Informationen zum Thalamus
    \item
    Kollaterale bilden Efferenzkopie zum Zerebellum (Soll-Zustand, der dann im Kleinhirn mit Ist-Zustand verglichen wird) 
\end{itemize}
...
\begin{block}{Kortiko-retikuläre Bahn}

Vor allem vom prämotorischen Kortex zur Formatio reticularis 
\end{block}
...
\begin{frame}{Läsionen der deszendierenden Bahnen}
    
    \begin{block}{Läsion der Pyramidalbahn}
    Verlust des Präzisionsgriffs, pathologische Babinskireflexe
    
    \end{block}

    \begin{block}{Läsion der Capsula Interna}
    Wesentlich drastischer da efferente \textbf{und} afferente Wege betrofen sind. Früh oft schlaffe Lähmung (später spastische Versteifung) der kontralateralen Extrimität. Im Extremfall (bei beidseitiger Läsion) Locked In Syndrom
...
\begin{frame}{Afferenzen zum motorischen Kortex}

Afferenzen liefern kontinuierlich aktuelle Informationen über den aktuellen Zustand von Körper und Umgebung, sodass Bewegungen optimal angepasst werden können.


Eingänge:
\begin{itemize}
    \item 
    Assoziationsfasern (ipsilateral)
    \item
    Kommissurenfasern (kontralateral)
    \item
    Basalganglien
    \item
    Zerebellum
    ...
    Zingulärer Kortex
    \item
    Locus coeruleus (Noradrenalin stimuliert Aufmerksamkeit)
...
%% FINAL version: comment from here
\begin{frame}{Transkortikale Reflexe}

\begin{itemize}
\item
Funktionieren wie "normale" Dehnungs- und Spannungsreflexe, aber \dots
    \item 
    Betreffen Neuronen, die die Finger innervieren (oft monosynaptische Verbindungen von Betzschen Riesenzellen, sodass spinale Reflexe nicht stattfinden können)
    \item
    Sensorische Afferenzen ziehen vom Rückenmark zu S1, von dort zu M1, und wieder zum Rückenmark
    \item
    \textcolor{theme}{Wie schnell sind solche transkortikale Reflexe?}
\end{itemize}
...
Funktionieren wie "normale" Dehnungs- und Spannungsreflexe, aber \dots
    ...
    Betreffen Neuronen, die die Finger innervieren (oft monosynaptische Verbindungen von Betzschen Riesenzellen, sodass spinale Reflexe nicht stattfinden können)
    \item
    Signal durch sensorische Afferenzen vom Rückenmark zu S1, von dort zu M1, und wieder zum Rückenmark
    \item
Langsam! 60-100 ms (auch "Long Loop Reflexe" genannt) \pause (Aber trotzdem noch schneller als Willkürbewegungen!)
\end{itemize}

\end{frame}


{ \usebackgroundtemplate{\includegraphics[width=1.2\paperwidth]{jigsaw.jpg}} 
\begin{frame}{Ja ok, aber \dots \pause  Wie passt das alles zusammen?}
...
\section{Zusammenspiel motorischer Strukturen bei Willkürbewegungen}
 ...
 \begin{frame}{Vorbereitung der Bewegung}
    
\begin{block}{Bereitschaftspotential}

\begin{itemize}
    \item 
    0.8\,s for Bewegung im Kortex messbar
    \item
    50\,ms vor Bewegung im M1
    \item
    Potentiale im sensorischen Kortex sind erst nach der Bewegung messbar (Feedback)
\end{itemize}
...
\begin{block}{Bewegungsvorbereitung und Umwsetzung}

\begin{itemize}
    \item 
    Handlungsantrieb: Limbisches System, Assoziationskortex
    \item
    Entschlussfindung und Strategieentwicklung: SMA, PMA, Basalganglien
    \item
    Planung/Programmierung: Basalganglien, SMA, PMA, Kleinhirn
    \item
    Umsetzung: M1 (Stärke, Richtung und Dynamik der Bewegung)
    \end{itemize}
...
\end{block}    
 ...
 \begin{frame}{(Grobe) Hierarchie der Bewegungszentren}
 
 \begin{center}
\includegraphics[width=0.8\textwidth]{motorhierarchie.png}
 \end{center}
...
{ \usebackgroundtemplate{\includegraphics[width=1\paperwidth]{gehen.png}} 
\begin{frame}
...
Hirnstamm. Von Patrick J. Lynch, variation by User:Hk kng - derivative of File:Brain human sagittal section.svg by Patrick J. Lynch, medical illustrator, CC BY-SA 3.0, \url{https://commons.wikimedia.org/w/index.php?curid=6998225}
...
\item
Kleinkind lernt gehen. Photo by \href{https://unsplash.com/@nate_dumlao?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText}{Nathan Dumlao} on \href{https://unsplash.com/s/photos/toddler?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText}{Unsplash}
...
Person schenkt sich Wasser ein. Photo by \href{https://unsplash.com/@sanengineer?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText}{Ikhsan Sugiarto} on \href{https://unsplash.com/s/photos/water-glass?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText}{https://unsplash.com/@sanengineer?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText}
  ...
  \item
  Puzzleteile. Photo by \href{https://unsplash.com/@sloppyperfectionist?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText}{Hans-Peter Gauster} on \href{https://unsplash.com/s/photos/jigsaw?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText}{Unsplash}
  

\item
Pyramidenbahn. Von Henry Vandyke Carter - Henry Gray (1918) Anatomy of the Human Body, Bartleby.com: Gray's Anatomy, Tafel 764, Gemeinfrei, \url{https://commons.wikimedia.org/w/index.php?curid=541610}
